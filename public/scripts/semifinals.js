...
function populateWinners() {
    const matchups = [
        { id: 'east-match1-winner', teams: ['Hawks', 'Celtics'] },
        { id: 'east-match2-winner', teams: ['Bucks', 'Sixers'] },
        { id: 'west-match1-winner', teams: ['Lakers', 'Nuggets'] },
        { id: 'west-match2-winner', teams: ['Warriors', 'Suns'] }
    ];
...
function submitSemifinals() {
    ...
    .then(data => {
        ...
        if (data.error) {
            ...
            window.location.href = '/conferencefinals.html'; // Next step in sequence
        }
    })
    .catch(error => {
